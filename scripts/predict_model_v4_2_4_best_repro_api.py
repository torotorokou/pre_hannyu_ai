# -*- coding: utf-8 -*-
"""
predict_model_v4_2_4_best_repro_api.py  (完全修正版)

- 二段構成：Stage1(品目のOOFスタッキング) → Stage2(合計GBDT) + 動的ブレンド
- 特徴量：曜日/週番号の周期(COS/SIN)、予約データ、合計ラグ/移動平均、品目ラグ/移動平均
- 交互作用：(*_pred)×曜日ダミー、予約(件数/台数/固定比)×曜日ダミー
- ガード：予約ゼロ×週末/祝日前後キャップ、曜日別線形キャリブレーション
- 残差バイアス補正：曜日×直近W日の分位(既定は中央値)＋上限キャップ
- ブレンドの“direct側”を p50/平均 で切替 (--blend-use-p50)
  ※ 履歴側の direct も p50/平均 で統一（未来リークなし）
- モデル保存：推論向けバンドル(Stage1 packs + Stage2 + 直近履歴)を joblib で保存
- 断片化対策：交互作用列は dict に貯めて最後に concat（高速・警告抑制）
"""
...
pd.options.mode.copy_on_write = True  # 断片化の悪化を抑止（サポート環境で有効）
...
class Config:
    ...
    # ブレンド direct の基準（True=>p50, False=>平均(ls)）
    blend_use_p50: bool = False
    # 保存関連
...
# =========================
# Stage2 matrix (interactions)  — 高速・安全版
# =========================
def make_stage2_matrix(df_in: pd.DataFrame, target_items: List[str], cfg: Config) -> Tuple[pd.DataFrame, List[str]]:
    # 1) 「合計」「__date__」などは除去
    drop_cols = [c for c in ["合計", "__date__"] if c in df_in.columns]
    X = df_in.drop(columns=drop_cols).copy()

    # 2) datetime 列を念のため除去
    # pandas 1.x/2.x 互換: "datetime64[ns, tz]" は無効なので "datetime64" のみで判定
    dt_cols = list(X.select_dtypes(include=["datetime64"]).columns)
    if dt_cols:
    ...
    # 4) 交互作用を dict に一括生成して最後に concat
    newcols: Dict[str, np.ndarray] = {}

    cols_pred = [f"{it}_pred" for it in target_items if f"{it}_pred" in X.columns]
    if cfg.add_dow_item_interactions and "dow" in X.columns:
        dow_i = X["dow"].astype(int)
        d = pd.get_dummies(dow_i, prefix="dow", drop_first=False)
        ...
        for c in cols_pred:
            if c in X.columns:
                ...
                for k in d.columns:
                    dv = d[k].to_numpy(dtype=float, copy=False)
                    newcols[f"{c}__{k}"] = xv * dv

        for base in ["reserve_count", "reserve_sum", "fixed_ratio"]:
            if base in X.columns:
                bv = X[base].to_numpy(dtype=float, copy=False)
                for k in d.columns:
                    dv = d[k].to_numpy(dtype=float, copy=False)
                    newcols[f"{base}__{k}"] = bv * dv
    ...
    for c in feature_names:
        if not np.issubdtype(X[c].dtype, np.floating):
            X[c] = X[c].astype(float, copy=False)
...
def _resid_bias_adjustment(pred_day: pd.Timestamp,
    ...
    try:
        ...
        chosen_key = "p50" if cfg.blend_use_p50 else "ls"
        direct_hist = np.ravel(models_total[chosen_key].predict(Xt_hist))
...
def run_walkforward(df_raw: pd.DataFrame,
    ...
    for i, pred_day in enumerate(idx):
        ...
        # ---- ダイナミックブレンド（direct側を p50/平均で統一）
        hist_df_proc, _ = make_stage2_matrix(hist_df, target_items, cfg)
        y_hist = hist_df["合計"].values.astype(float)
        sum_hist = hist_df[[c for c in hist_df.columns if c.endswith("_pred")]].sum(axis=1).values.astype(float)
        chosen_key = "p50" if cfg.blend_use_p50 else "ls"
        direct_hist = models_total[chosen_key].predict(
        ...
        base_for_blend = p50 if cfg.blend_use_p50 else mean_pred
        total_pred_today = float(w * base_for_blend + (1 - w) * sum_items_today)
    ...
    try:
        ...
        row = {
            ...
            "blend_use_p50": cfg.blend_use_p50,
...
# =========================
def main():
    ap = argparse.ArgumentParser(description="品目OOF→合計direct/ブレンド（再現＋残差補正＋交互作用強化＋モデル保存）")
    ...
    # ブレンド基準
    ap.add_argument("--blend-use-p50", action="store_true",
                    help="Trueにするとブレンドのdirect側にp50(中央値)を用いる（MAEを下げやすい）")
    ...
    cfg = Config(
        ...
        blend_use_p50=args.blend_use_p50,
